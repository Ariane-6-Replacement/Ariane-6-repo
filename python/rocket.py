...
class Rocket():
    def __init__(self, input_dict):
        ...
        self.mf1 = float(input_dict["mf1"].get())
        self.mf2 = float(input_dict["mf2"].get())
        self.isp2 = float(input_dict["ISP2"].get())
        self.root = None
...
if __name__ == "__main__":
    ...
    i = 0
    ttk.Label(root, text="General properties", font=custom_font).grid(column=1, row=i)
    ...
    ttk.Combobox(root, textvariable=input_dict["orbit"], values=orbit_options, state="readonly", width=17).grid(column=1, row=i)
    ...
    ttk.Entry(root, textvariable=input_dict["payload"], width=20).grid(column=1, row=i)
    ...
    i += 1
    ttk.Label(root, text="Inert mass fraction 2nd stage").grid(column=0, row=i)
    input_dict["mf2"] = tk.StringVar(value='0.04')
    ttk.Entry(root, textvariable=input_dict["mf2"]).grid(column=1, row=i)

    i += 1
    ttk.Label(root, text="ISP 2nd stage").grid(column=0, row=i)
    input_dict["ISP2"] = tk.StringVar(value='296')
    ttk.Entry(root, textvariable=input_dict["ISP2"]).grid(column=1, row=i)

    #1st stage props ######################
    i += 1
    ttk.Label(root, text="1st stage properties", font=custom_font).grid(column=1, row=i)
    # dV_fraction slider
    i += 1
    ttk.Label(root, text="dV fraction stage 1:").grid(column=0, row=i)
    slider = ttk.Scale(root, from_=0, to=1, orient="horizontal", length=150, command=update_value)
    ...
    ttk.Combobox(root, textvariable=input_dict["engine"], values=engine_options, state="readonly", width=17).grid(column=1, row=i)

    input_dict["engine"].set("Prometheus")  # default value

    i += 1
    ttk.Label(root, text="Inert mass fraction 1st stage").grid(column=0, row=i)
    input_dict["mf1"] = tk.StringVar(value='0.05')
    ttk.Entry(root, textvariable=input_dict["mf1"]).grid(column=1, row=i)
    # Boostback
    i += 1
    ...
    ttk.Combobox(root, textvariable=input_dict["material_tank"], values=material_options, state="readonly", width=17
                 ).grid(column=1, row=i)
    ...
    material_options = list(m.keys())
    ttk.Combobox(root, textvariable=input_dict["material_misc"], values=material_options, state="readonly", width=17
                 ).grid(column=1, row=i)
    ...
    ttk.Combobox(root, textvariable=input_dict["bulkhead"], values=bulkhead_options, state="readonly", width=17
                 ).grid(column=1,row=i)
